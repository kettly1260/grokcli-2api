...
```bash
docker pull ghcr.io/hm2899/grokcli-2api:1.9.48
# 或
...
services:
  ...
  grokcli-2api:
    image: ghcr.io/hm2899/grokcli-2api:1.9.48
    ports:
...
git tag v1.9.48
git push origin v1.9.48
```

成功后可拉取（**必须小写**）：

- `ghcr.io/hm2899/grokcli-2api:1.9.48`
...
turnstile-solver/                     # 本地过盾（默认内联进主容器；懒加载 + 空闲回收）
...
- **v1.9.48**（当前）：
  - **注册进度恢复**：刷新 / soft-nav 后自动从后端恢复进行中的协议注册，进度日志不再空白
  - **任务日志详情**：刷新后保留选中行详情（不再依赖脆弱 HTML 属性）
  - **空闲回收加固**：Turnstile 浏览器关闭更彻底；`in_flight` 泄漏可强制回收；新增 `/health` 与 `/reclaim`
  - 管理台模型列表走专用 `/models` 目录加载，避免大池 `/dashboard` 导致空白
  - API 转发 / Token 续期 / 模型测活路径不受影响
- **v1.9.47**：
...
> 镜像 tag 与 `app.py` 中 `APP_VERSION` 一致（当前 **1.9.48**）。推 `main` 会打 `edge` 与版本号；打 `v1.9.48` tag 会额外发布 `latest`。  
