...
func TestJoinShellArgvMultiStatement(t *testing.T) {
	...
	if joinShellArgv([]string{one}, DialectPOSIX) != one {
...
func TestProjectShellArgsPreservesWorkdir(t *testing.T) {
	in := "{\"command\":\"Select-String -Path workout-core.js -Pattern 'isPlaying'\",\"workdir\":\"G:\\\\LLM\\\\rehab\"}"
	// Build via map to avoid escape confusion.
	payload, err := json.Marshal(map[string]any{
		"command": "Select-String -Path workout-core.js -Pattern 'isPlaying'",
		"workdir": `G:\LLM\rehab`,
	})
	if err != nil {
		t.Fatal(err)
	...
	if obj["cmd"] != "Select-String -Path workout-core.js -Pattern 'isPlaying'" {
		t.Fatalf("cmd rewritten or lost: %v", obj["cmd"])
	}
	if obj["workdir"] != `G:\LLM\rehab` {
		t.Fatalf("workdir lost: %v in %s", obj["workdir"], out)
	}
	if _, ok := obj["command"]; ok {
		t.Fatalf("command key should be projected away: %s", out)
	}
}

func TestProjectShellArgsPassthroughNoBashRewrite(t *testing.T) {
	// bash glue must pass through unchanged
	cmdIn := `echo 'it'\''s fine'`
	payload, _ := json.Marshal(map[string]any{"command": cmdIn, "workdir": "/tmp"})
	out := ProjectShellArgsForClient(string(payload), "exec_command", "cmd")
	var obj map[string]any
	_ = json.Unmarshal([]byte(out), &obj)
	if obj["cmd"] != cmdIn {
		t.Fatalf("passthrough failed: got %q want %q", obj["cmd"], cmdIn)
	}
	if obj["workdir"] != "/tmp" {
